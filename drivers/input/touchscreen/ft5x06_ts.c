...
static int __init ft5x0x_ts_init(void)
{
	...
	//type = 0;
        ...
        // first lvds
        ret = gpio_request(TP_IOCTL1, "TP_IOCTL1");
        if (ret) {
                gpio_free(TP_IOCTL1);
                ...
                if(ret)
                {
                        printk("ft5xox: Failed to request TP_IOCTL1 \n");
                }
        }
        gpio_direction_output(TP_IOCTL1, 0);
        mdelay(200);

        gpio_direction_output(TP_IOCTL1, 1);
       ...
       //second lvds
        ret = gpio_request(TP_IOCTL2, "TP_IOCTL2");
        if (ret) {
                gpio_free(TP_IOCTL2);
                ...
                if(ret)
                {
                        printk("ft5xox: Failed to request TP_IOCTL2 \n");
                }
        }
        gpio_direction_output(TP_IOCTL2, 0);
        mdelay(200);
	
        gpio_direction_output(TP_IOCTL2, 1);
        ...
        //add by dg 2015-08-21
        gpio_request(SABRESD_CAP_TCH_INT0, "touch_int0");
        gpio_direction_input(SABRESD_CAP_TCH_INT0);
        gpio_free(SABRESD_CAP_TCH_INT0);


        gpio_request(SABRESD_CAP_TCH_INT1, "touch_int1");
